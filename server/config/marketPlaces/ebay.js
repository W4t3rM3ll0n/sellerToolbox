...
ebay.acceptToken = (code, user, res, callback) => {
...
ebay.refreshToken = (refToken, user, res, callback) => {
  const opt = ebayHttpOptions(
    '/identity/v1/oauth2/token',
    'POST', {
      'Content-Type': 'application/x-www-form-urlencoded',
...
// Opt in to SPM
ebay.optInSPM = (eToken, res, callback) => {
  const opt = ebayHttpOptions(
...
// Get opt in programs
ebay.getOptInPrograms = (eToken, res, callback) => {
  const opt = ebayHttpOptions(
...
ebay.getPolicies = (eToken, res, callback) => {
...
ebay.testCall = (eToken, res, callback) => {
  const opt = ebayHttpOptions(
...
ebay.getOffers = (eToken, res, callback) => {
  const opt = ebayHttpOptions(
...
ebay.createOrReplaceInventory = (eToken, products, sku, res, callback) => {
  const opt = ebayHttpOptions(
