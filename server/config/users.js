...
// Add a user
users.addUser = (regUser, callback) => {
...
// This is not in use.
users.getUserById = (user, callback) => {
...
// Update user
users.updateUser = (user, updateUser, callback) => {
...
users.addUpdateAddress = (address, userId, callback) => {
  const updateList = [];
  const newList = [];
...
// Delete an address
users.deleteAddress = (addressId, userId, callback) => {
...
users.updatePassword = (user, updatePassword, callback) => {
  bcrypt.genSalt(10, (err, saltRounds) => {
...
// Delete a user
users.deleteUser = (userId, callback) => {
