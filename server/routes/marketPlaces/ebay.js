...
const configEbay = require('../../config/marketplaces/ebay');
...
router.get('/tokens', passport.authenticate('jwt', { session: false }), (req, res) => {
  res.json({
...
router.post('/refreshToken', passport.authenticate('jwt', { session:false }), (req, res) => {
...
router.post('/spm', passport.authenticate('jwt', { session:false }), (req, res) => {
  // Ebay Auth Token
...
router.get('/getOptedInPrograms', passport.authenticate('jwt', { session:false }), (req, res) => {
  const eToken = req.user.ebayauthtoken;
...
// Get Ebay Policies
router.get('/getEbayPolicies', passport.authenticate('jwt', { session:false }), (req, res) => {
  const eToken = req.user.ebayauthtoken;
...
router.get('/testCall', passport.authenticate('jwt', { session:false }), (req, res) => {
  const eToken = req.user.ebayauthtoken;
...
router.get('/getOffers', passport.authenticate('jwt', { session:false }), (req, res) => {
  const eToken = req.user.ebayauthtoken;
...
router.put('/createOrReplaceInventory', passport.authenticate('jwt', { session:false }), (req, res) => {
  const eToken = req.user.ebayauthtoken;
